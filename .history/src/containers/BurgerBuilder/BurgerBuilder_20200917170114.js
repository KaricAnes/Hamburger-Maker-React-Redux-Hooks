...
const BurgerBuilder = (props) => {
  ...
  */
  ...
  const onInitIngredients = useCallback(
    ...
    //Da nema ovog useCallbacka -> stalno bi se okidao onInitIngredients() i setao ih sve na {ing:0}
    () => dispatch(actionCreators.fetchIngredients()),
    [dispatch]
  );

  const onPurchaseInit = () => dispatch(actionCreators.purchaseInit());
  // orderR.js kada se okine PURCHASE_BURGER_SUCCESS purchased bude setano na true i mi budemo redirektani na pocetnu
  //orderA.js purchaseInit --> purchaseInit
  //orderR.js  purchaseInit -> vraca purchased na false, da ne bismo bili onemoguceni da udjemo na checkout nakon jedne uspjesne narudzbe
  ...
  useEffect(() => {
    onInitIngredients(); //vraca i bulding na false
  }, [onInitIngredients]);
  ...
  const purchaseHandler = () => {
    ...
    if (isAuth) {
      ...
      onSetAuthRedirectPathBB("/checkoutt");
      /*ovo je jedino mjesto gdje se RedirectPathBB seta na '/checkoutt'. Zato i ne mozemo
      da kliknemo na vise pa na manje i tako setamo building na true i onda odemo na Auth.js
      direktno i da se logujemo i odemo na /checkout 'prazni'.
      Fazon je sto na purchaseHandler ne mozemo svakako kliknuti ako nismo buildali burger.
      Sad mi je jasno i zasto ne mozemo da unesemo u url /checkout kada smo nabildali burger. 
      Djaba nam je to kada nismo put setali na '/checkout jer ' */
      //--> u Auth.js-u je authRedirect u returnu. building je u BBreduceru na 2 mejsta
      //ne mozemo kliknuti na button ako nismo bildali. Zelimo da se autentifikujemo pa da idemo na checkout odmah. A ne da opet idemo nazad na bildanje burgera.
